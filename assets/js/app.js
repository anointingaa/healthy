...
const urgentQs=[
  "Severe or persistent headache that won’t go away",
  "Severe dizziness or fainting",
  "Sudden swelling of face or hands",
  "Shortness of breath, chest pain, or very fast heartbeat",
  "Severe nausea/vomiting such that you can’t keep fluids down",
  "Severe abdominal pain",
  "Baby’s movements have stopped or slowed (after 28 weeks)",
  "Vaginal bleeding or water breaking early"
];

const routineQs=[
  "I’ve had a prenatal visit (or have one scheduled)",
  "I have done malaria test and received treatment if positive",
  "I have done HIV test and know my status",
  "I have done hepatitis B test",
  "I have done syphilis test",
  "I have checked my blood group and genotype",
  "I take a daily prenatal vitamin (with folate/folic acid)",
  "I stay active with pregnancy-safe exercise",
  "I have access to enough nutritious food",
...
function renderQuestions(){
  ...
  urgentQs.forEach((q,i)=>{
    const idYes=`u${i}-yes`,idNo=`u${i}-no`;
    const row=document.createElement('div');
    row.className="bg-rose-50 border border-rose-200 rounded-lg p-3";
    row.innerHTML=`<p class='text-sm font-medium'>${i+1}. ${q}</p>
      <div class='mt-2 flex gap-4 text-sm'>
        <label class='inline-flex items-center gap-1'><input type='radio' name='u${i}' id='${idYes}' value='yes'> Yes</label>
        <label class='inline-flex items-center gap-1'><input type='radio' name='u${i}' id='${idNo}' value='no' checked> No</label>
      </div>`;
    urgentEl.appendChild(row)
  });
  ...
  routineQs.forEach((q,i)=>{
    ...
    row.className="flex items-start gap-3 bg-slate-50 border border-slate-200 rounded-lg p-3";
    row.innerHTML=`<input type='checkbox' id='${id}' class='mt-1'> <span class='text-sm'>${q}</span>`;
    routineEl.appendChild(row)
  })
}

function explainRoutine(item){
  switch(item){
    case'I have done malaria test and received treatment if positive':return"Malaria during pregnancy can cause miscarriage, anemia, and low birth weight. Test and treat early at a government-approved clinic.";
    case'I have done HIV test and know my status':return"Knowing your status allows timely treatment to protect you and your baby from infection.";
    case'I have done hepatitis B test':return"Hepatitis B can pass to your baby during delivery; early detection allows preventive measures for your newborn.";
    case'I have done syphilis test':return"Untreated syphilis can cause stillbirth or birth defects; treatment is simple and effective when done early.";
    case'I have checked my blood group and genotype':return"Essential to prevent problems like rhesus incompatibility and to understand sickle cell disease risk.";
    case'I take a daily prenatal vitamin (with folate/folic acid)':return"Folic acid helps prevent birth defects; iron supports you against anemia.";
    case'I stay active with pregnancy-safe exercise':return"Moderate activity improves circulation, mood, and sleep if your clinician says it’s safe.";
    case'I have access to enough nutritious food':return"Good nutrition supports your health and your baby’s growth; ask your clinic about local support if needed.";
    case'I feel safe and supported at home':return"A safe, supportive environment reduces stress and risk of harm. Seek help if you feel unsafe.";
    case'I’ve had a prenatal visit (or have one scheduled)':return"Early and regular antenatal visits help catch issues early and support a healthy pregnancy.";
    default:return"Discuss this item with your healthcare provider for personalised advice."
  }
}
...
function buildEmailLong(dueInfo){
  ...
  const lines=[];
  lines.push("Hello,","");
  lines.push("Here is your Healthy With Happiness pregnancy check summary:","");
  lines.push(`Estimated Due Date: ${formatDate(dueInfo.due)}`,"");

  if(urgentList.length>0){
    lines.push("⚠ Urgent Concerns:");
    lines.push("If you have any of the following symptoms, please seek immediate medical care at the nearest qualified health facility:");
    urgentList.forEach(symp=>{
      let why="This could indicate a serious pregnancy complication and needs urgent care.";
      if(symp.includes("headache"))why="May indicate preeclampsia, very high blood pressure, or other dangerous conditions.";
      if(symp.includes("dizziness"))why="Could be due to anemia, low blood pressure, or dehydration.";
      if(symp.includes("swelling"))why="Possible sign of preeclampsia—this can be dangerous if untreated.";
      if(symp.includes("Shortness of breath")||symp.includes("chest pain"))why="May indicate a heart or lung problem—get urgent assessment.";
      if(symp.includes("nausea/vomiting"))why="Can cause dehydration and harm your baby if untreated.";
      if(symp.includes("abdominal"))why="Could be preterm labour or placental problems—needs evaluation.";
      if(symp.includes("movements"))why="Could mean baby is in distress—seek urgent care.";
      if(symp.includes("Vaginal bleeding")||symp.includes("water"))why="Could mean labour or other complications—get seen immediately.";
      lines.push(`- ${symp}: ${why}`)
    });
    lines.push("")
  }
  ...
  if(gaps.length>0){
    lines.push("🔍 Routine Checks to Complete:");
    lines.push("These tests and steps are especially important in Nigeria and across Africa:");
    gaps.forEach(item=>{
      const label=item.startsWith("I ")?item.replace(/^I /,""):item;
      lines.push(`- ${label}: ${explainRoutine(item)}`)
    });
    lines.push("")
  }

  if(phqFlag){
    lines.push("🧠 Mood Check:");
    lines.push("Your mood responses suggest checking in with a clinician or counsellor could be helpful. If you have thoughts of harming yourself, seek urgent care immediately.","")
  }

  lines.push("This guidance is educational and not a substitute for medical advice. Always consult a qualified health professional.","","— Healthy With Happiness Team");
  return lines.join("\n")
}
...
function updateStepper(currentStep){
  const steps=[document.getElementById('step1-ind'),document.getElementById('step2-ind'),document.getElementById('step3-ind')];
  steps.forEach((el,idx)=>{
    if(!el) return;
    if(idx===currentStep){el.className='badge badge-emerald';el.style.background='#059669';el.style.color='#fff'}
    else{el.className='badge badge-muted';el.style.background='';el.style.color=''}
  })
}
...
<div class="bg-slate-50 border border-slate-200 rounded-lg p-3">
  ...
  // SUBSCRIBE: send detailed email via Systeme (with chunking)
  ...
  subBtn?.addEventListener('click', async ()=>{
    ...
    chunks.slice(0,8).forEach((t,i)=>{ meta[`long_${i+1}`]=t });
    ...
    try{
      ...
      if(res?.message && res.message.includes('Success')){
        ...
        console.error("Subscription error:", res);
        if(msg){
